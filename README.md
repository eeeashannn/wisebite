...
Phone browsers block camera access on insecure origins such as `http://192.168.x.x:3000`. You must open the app over HTTPS.

### Local same-Wi-Fi testing (manual entry + app features)

1. Start backend on port `5050`:
   - From `backend/`: `python app.py`
2. Start frontend dev server:
   - From `frontend/`: `npm start`
3. Open `http://<your-lan-ip>:3000` on phone.

This mode is useful for general testing, but camera scanning may be blocked on many phones because it is HTTP.

### Free phone camera deployment (Cloudflare Pages + Cloudflare Tunnel)

This keeps backend on your laptop and serves frontend from HTTPS.
...
4. Build frontend with the tunnel URL:
   - From `frontend/`: `REACT_APP_API_URL=https://<your-tunnel-url> npm run build:pages`
   - You can use `frontend/.env.pages.example` as a template for variables.
5. Deploy frontend to Cloudflare Pages:
   - From `frontend/`: `CF_PAGES_PROJECT=<your-pages-project> npm run deploy:pages`
   - If running in CI/non-interactive shells, set `CLOUDFLARE_API_TOKEN` first. For local interactive use, run `wrangler login`.
6. Open the Pages URL (`https://...pages.dev`) on phone and allow camera permission.

### Operational notes

- Keep your laptop running with both `python app.py` and `cloudflared`; otherwise API calls fail while frontend still loads.
- Quick tunnels rotate URLs when restarted. Rebuild/redeploy if `REACT_APP_API_URL` changes.
- Do not use `REACT_APP_API_URL=http://127.0.0.1:5050` for phone testing.

### CORS

With default `FLASK_RUN_HOST=0.0.0.0`, backend CORS is permissive for local development. For strict allowlists, set `FLASK_RUN_HOST=127.0.0.1` and configure `WISEBITE_CORS_ORIGINS`.